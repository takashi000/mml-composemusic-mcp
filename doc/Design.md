...
> 作成日: 2026-07-11 | 更新日: 2026-07-12 | ステータス: **第2段階設計（パーサー再設計済み）**
...
>
> **WAVファイル名**: 固定名 `output.wav` ではなく、生成時刻を含む `output_YYYYMMDD_HHMMSS_mmm.wav` 形式で出力される。
...
（各イベントのフィールドは第1段階実装と同じ）

---

## 5. MMLパーサ設計

### 5.1 新アーキテクチャ: 4フェーズ分離
...
| 旧コード | 新コード | 理由 |
|---|---|---|
| `SYNTAX_VALUE_OUT_OF_RANGE` | `SEMANTIC_VALUE_OUT_OF_RANGE` | 値範囲は意味エラー |
| `SYNTAX_NOTE_OUT_OF_RANGE` | `SEMANTIC_NOTE_OUT_OF_RANGE` | 音域は意味エラー |
| `SYNTAX_CHANNEL_MISMATCH` | `SEMANTIC_CHANNEL_MISMATCH` | チャンネル適合性は意味エラー |
| `SYNTAX_EMPTY_TRACK` | `SEMANTIC_EMPTY_TRACK` | 空トラックは意味エラー |
| `SYNTAX_OUTSIDE_TRACK` | `SEMANTIC_OUTSIDE_TRACK` | トラック外コマンドは意味エラー |
| `SYNTAX_UNDEFINED_REFERENCE` | `SEMANTIC_UNDEFINED_REFERENCE` | 未定義参照は意味エラー |
| `SYSTEM_SYNTHESIS_FAILED` | `RUNTIME_SYNTHESIS_FAILED` | 実行時エラー |
| `SYSTEM_WAV_WRITE_FAILED` | `RUNTIME_WAV_WRITE_FAILED` | 実行時エラー |
| `SYSTEM_INTERNAL_ERROR` | `RUNTIME_INTERNAL_ERROR` | 実行時エラー |

### 7.4 回復戦略

| 状況 | 動作 |
|---|---|
| ERROR時 | そのフェーズで解析を停止し、エラーを返す |
...
詳細は [plans/mml-parser-refactor.md](plans/mml-parser-refactor.md) 第3章を参照。
...
<ppmck_statement>
              ::= <note> | <rest> | <octave_cmd> | <length_cmd>
                | <volume_cmd> | <duty_cmd> | <tempo_cmd> | <tie_cmd>
...
| **デューティ比** | `q0`〜`q3` で4種 | `@1`(50%), `@2`(25%) の2種のみ |
...
| Lexer | ppmck/pyxel 両モード対応、共通トークン定義、INVALIDトークン |
| Parser | ppmck/pyxel 別Parser、AST生成 |
| SemanticAnalyzer | ppmck/pyxel 別Analyzer、AST→IR変換 |
| IR | NoteSequence、各種Event、ErrorDetail（phase付き） |
